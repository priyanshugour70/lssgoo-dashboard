...
import { updateRoleSchema } from '@/types/rbac';
...
export async function GET(
  ...
  try {
    ...
    const result = await rbacService.getRole(params.id);
...
export async function PATCH(
  ...
  try {
    ...
    const result = await rbacService.updateRole(params.id, validated, context.userId);
...
export async function DELETE(
  ...
  try {
    ...
    await rbacService.deleteRole(params.id, context.userId);

    return Response.json(successResponse({ message: 'Role deleted successfully' }));
  } catch (error: any) {
    if (error.message === ErrorCodes.UNAUTHORIZED || error.message === ErrorCodes.FORBIDDEN) {
      return Response.json(
        errorResponse(error.message, 'Unauthorized'),
